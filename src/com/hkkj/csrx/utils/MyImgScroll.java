...
public class MyImgScroll extends ViewPager {
	...
	TextView textView;
	ArrayList<HashMap<String, String>> myimgarray=new ArrayList<HashMap<String,String>>();
	int mScrollTime = 0;
	Timer timer;
	int oldIndex = 0;
	int curIndex = 0;
			...
			this.setOnTouchListener(new OnTouchListener() {
				public boolean onTouch(View v, MotionEvent event) {
					if (event.getAction() == MotionEvent.ACTION_UP) {
						textView.setText(myimgarray.get(curIndex).get("name"));
						startTimer();
					} else {
						stopTimer();
					}
