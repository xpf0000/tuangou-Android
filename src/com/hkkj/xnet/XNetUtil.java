...
public class XNetUtil {
        ...
        obj
                ...
                .subscribe(new Subscriber<HttpResult<T>>() {
                    ...
                    public void onError(Throwable e) {
                        ...
                        {
                            e.printStackTrace();
                            Toast.makeText(location.context, e.toString(), Toast.LENGTH_SHORT).show();
                        }
                        else
                        {
                            XActivityindicator.showToast(location.context.getResources().getString(R.string.intent_error));
                        }
                        res.onError(e);
    ...
    private static class HttpResultFuncBool<T> implements Func1<HttpResult<T>, Boolean> {
        ...
        public Boolean call(HttpResult<T> httpResult) {
            ...
            if (httpResult.getStatus() != 1 && fail != null) {

                XActivityindicator.create(location.context).showErrorWithStatus(fail);

                return false;
            }
            else
            {
                if(httpResult.getInfo() != null && fail != null)
                {
